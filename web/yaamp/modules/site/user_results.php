...
if ($symbol == 'all') $users = getdbolist('db_accounts', "balance>.001 OR id IN (SELECT DISTINCT userid FROM workers) ORDER BY balance DESC");
else
{
...
foreach ($users as $user)
{
    ...
    {
        $user_coin = getdbosql('db_coins', "id=:id", array(
            ':id' => $user->coinid
        ));
        if ($user_coin)
        {
