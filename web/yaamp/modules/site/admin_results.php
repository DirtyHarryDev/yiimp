...
if (!empty($server)) {
    $coins = getdbolist('db_coins', "(installed OR enable OR watch) AND rpchost=:server ORDER BY algo, index_avg DESC", array(
        ':server' => $server
    ));
} else
...
foreach ($coins as $coin) {
    ...
    if (!$coin->enable)
        echo '<span class="hidden" title="Coin disabled">X</span>';
    else if ($coin->auto_ready)
        echo '<span class="green" title="Auto enable">A</span>';
    else
        echo '<span class="red" title="Stratum disabled">D</span>';

    if ($coin->visible)
        echo '<span title="Visible to public">V</span>';
    else
        echo '<span title="Hidden">H</span>';

    if ($coin->auxpow)
        echo '<span title="AUX PoW">X</span>';
    else
        echo '&nbsp;';
    ...
    if ($coin->rpccurl)
        echo '<span title="RPC with Curl">C</span>';
    else
        echo '&nbsp;';

    if ($coin->rpcssl)
        echo '<span title="RPC over SSL">S</span>';
    else
        echo '&nbsp;';
