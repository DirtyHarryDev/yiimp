...
$markets = getdbolist('db_markets', "name=:exch ORDER BY ((balance+ontrade)*price) DESC", array(
    ':exch' => $exch
));
...
foreach ($markets as $market) {
    if ($market->pricetime == 0)
        continue;
    $balance = $market->balance;
    $balance += $market->ontrade;
    if ($balance * $market->price2 < 200 * 1e-8)
        continue;
    ...
    if (arraySafeVal($symbols, $symbol))
