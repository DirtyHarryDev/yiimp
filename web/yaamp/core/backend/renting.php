...
function BackendRentingUpdate()
{
    ...
    foreach (yaamp_get_algos() as $algo) {
        ...
        dborun("update jobs set active=true where ready and price>$rent and algo=:algo", array(
            ':algo' => $algo
        ));
        dborun("update jobs set active=false where active and price<$rent and algo=:algo", array(
            ':algo' => $algo
        ));
    }
...
function BackendRentingPayout()
{
    ...
    foreach (yaamp_get_algos() as $algo) {
        $delay = time() - 5 * 60;
        dborun("delete from jobsubmits where status=2 and algo=:algo and time<$delay", array(
            ':algo' => $algo
        ));

        $amount = dboscalar("select sum(amount) from jobsubmits where status=1 and algo=:algo", array(
            ':algo' => $algo
        ));
        if ($amount < 0.00002000)
            continue;

        dborun("update jobsubmits set status=2 where status=1 and algo=:algo", array(
            ':algo' => $algo
        ));
        ...
        $total_hash_power = dboscalar("SELECT sum(difficulty) FROM shares where valid and algo=:algo", array(
            ':algo' => $algo
        ));
        if (!$total_hash_power)
            continue;

        $list = dbolist("SELECT userid, sum(difficulty) as total FROM shares where valid and algo=:algo GROUP BY userid", array(
            ':algo' => $algo
        ));
        ...
        $delay = time() - 5 * 60;
        dborun("delete from shares where algo=:algo and time<$delay", array(
            ':algo' => $algo
        ));
    }
